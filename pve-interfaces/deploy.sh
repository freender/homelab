...
deploy() {
    ...
    ssh "$host" "cd /tmp/homelab-pve-interfaces && chmod +x scripts/install.sh && if [ \"\$(id -u)\" -ne 0 ]; then echo 'Error: PVE/PBS deploy requires root SSH user' >&2; exit 1; fi && ./scripts/install.sh '$host'"
}
