...
class CacheEffectivenessStats:
    desired_units: int = 0
    desired_cached_units: int = 0
    desired_bytes: int = 0
    desired_cached_bytes: int = 0
...
def load_config(args: argparse.Namespace) -> Config:
    ...
    recent_movie_retention_days = int(os.environ.get("RECENT_MOVIE_RETENTION_DAYS", "14"))
    recent_tv_retention_days = int(os.environ.get("RECENT_TV_RETENTION_DAYS", "14"))
    ...
    if recent_movie_retention_days < 0:
        raise SystemExit("RECENT_MOVIE_RETENTION_DAYS must not be negative")
    if recent_tv_retention_days < 0:
        raise SystemExit("RECENT_TV_RETENTION_DAYS must not be negative")
...
def collect_units(root: Path, config: Config) -> dict[Path, tuple[int, float | None, float | None]]:
    units: dict[Path, tuple[int, float | None, float | None]] = {}
    for managed_root in config.managed_roots:
        ...
        for path in managed_path.rglob("*"):
            ...
            current_size, current_oldest, current_latest = units.get(unit, (0, None, None))
            oldest = stat_result.st_mtime if current_oldest is None else min(current_oldest, stat_result.st_mtime)
            latest = stat_result.st_mtime if current_latest is None else max(current_latest, stat_result.st_mtime)
            units[unit] = (current_size + stat_result.st_size, oldest, latest)
    return units
...
def collect_all_unit_stats(config: Config) -> dict[Path, UnitStats]:
    ...
    for unit in set(cache_units) | set(target_units):
        cache_size, cache_oldest, cache_latest = cache_units.get(unit, (0, None, None))
        target_size, target_oldest, target_latest = target_units.get(unit, (0, None, None))
...
def unit_latest_mtime(stats: UnitStats) -> float | None:
    latest_times = [
        value
        for value in (stats.latest_cache_mtime, stats.latest_tank_mtime)
...
def unit_recent_retention_days(relative_dir: Path, config: Config) -> int:
    if relative_dir.parts[0] in MOVIE_ROOTS:
        return config.recent_movie_retention_days
    return config.recent_tv_retention_days


def cached_recent_units(unit_stats: dict[Path, UnitStats], config: Config) -> set[Path]:
    now = time.time()
    recent_units: set[Path] = set()
    for relative_dir, stats in unit_stats.items():
        if stats.size_on_cache < 1:
            continue
        retention_days = unit_recent_retention_days(relative_dir, config)
        if retention_days < 1:
            continue
        latest_mtime = unit_latest_mtime(stats)
        if latest_mtime is None:
            continue
        if now - latest_mtime <= retention_days * 86400:
            recent_units.add(relative_dir)
    return recent_units
...
def sync_directory(
    relative_dir: Path,
    config: Config,
    stable_source_files: dict[Path, Path] | None = None,
) -> MoveResult:
    source_dir = config.source_root / relative_dir
    if stable_source_files is None:
...
def sync_tv_unit(
    relative_dir: Path,
    config: Config,
    source_files: dict[Path, Path] | None = None,
) -> MoveResult:
    ...
    for relative_dir, stats in sorted(unit_stats.items(), key=lambda item: str(item[0])):
        ...
        if relative_dir.parts[0] in MOVIE_ROOTS:
            ...
            if not stable_source_files:
                skipped_units += 1
                continue
            result = sync_directory(relative_dir, config, stable_source_files)
        else:
            stable_files = unit_file_map(config.source_root, relative_dir, config, stable_only=True)
            if not stable_files:
                skipped_units += 1
                continue
            result = sync_tv_unit(relative_dir, config, stable_files)
...
def build_cache_effectiveness_stats(
    ...
    desired_units: set[Path],
) -> CacheEffectivenessStats:
    desired_cached_units = 0
    desired_bytes = 0
    desired_cached_bytes = 0
    for relative_dir in desired_units:
        stats = unit_stats.get(relative_dir)
        if stats is None:
            continue
        desired_bytes += stats.total_size
        if stats.size_on_cache < 1:
            continue
        desired_cached_units += 1
        desired_cached_bytes += stats.size_on_cache
    return CacheEffectivenessStats(
        desired_units=len(desired_units),
        desired_cached_units=desired_cached_units,
        desired_bytes=desired_bytes,
        desired_cached_bytes=desired_cached_bytes,
    )


def classify_ondeck_units(entries: list[OnDeckEntry], desired_units: set[Path]) -> tuple[set[Path], set[Path]]:
    current_units = {entry.relative_dir for entry in entries if entry.current and entry.relative_dir in desired_units}
    prefetch_units = {entry.relative_dir for entry in entries if not entry.current and entry.relative_dir in desired_units}
    prefetched_only = prefetch_units - current_units
    return current_units, prefetched_only


def sort_units_by_age(
    units: set[Path],
    unit_stats: dict[Path, UnitStats],
    state: dict[str, dict[str, float]],
) -> list[Path]:
    return sorted(units, key=lambda unit: unit_age_key(unit, unit_stats.get(unit), state))
...
def report_cache_effectiveness(config: Config, args: argparse.Namespace) -> int:
    ...
    effectiveness = build_cache_effectiveness_stats(unit_stats, desired_ondeck)
    cache_units = {unit: stats for unit, stats in unit_stats.items() if stats.size_on_cache > 0}
    recent_units = cached_recent_units(unit_stats, config)
    _total_bytes, used_bytes, available_bytes = filesystem_usage(config.source_root)

    print(
        f"report: desired_ondeck_units={effectiveness.desired_units} "
        f"desired_ondeck_cached_units={effectiveness.desired_cached_units} "
        f"desired_ondeck_hit_rate={format_percent(effectiveness.desired_cached_units, effectiveness.desired_units)} "
        f"desired_ondeck_bytes={format_bytes(effectiveness.desired_bytes)} "
        f"desired_ondeck_cached_bytes={format_bytes(effectiveness.desired_cached_bytes)} "
        f"desired_ondeck_byte_hit_rate={format_percent(effectiveness.desired_cached_bytes, effectiveness.desired_bytes)} "
        f"recent_cached_units={len(recent_units)} "
        f"recent_movie_retention_days={config.recent_movie_retention_days} "
        f"recent_tv_retention_days={config.recent_tv_retention_days} "
    ...
    if not desired_ondeck:
        print("top_ondeck: none")
        return 0

    for index, relative_dir in enumerate(
        sorted(desired_ondeck, key=lambda unit: (-ondeck_scores.get(unit, 0), str(unit)))[: args.report_limit],
        start=1,
    ):
        stats = unit_stats.get(relative_dir)
        if stats is None:
            ...
            total_size = 0
        print(
            f"top_ondeck: rank={index} location={location} score={ondeck_scores.get(relative_dir, 0)} size={format_bytes(total_size)} unit={relative_dir}"
        )
    return 0
...
def merge_evict_results(current: EvictResult, delta: EvictResult) -> EvictResult:
    return EvictResult(
        current.evicted_units + delta.evicted_units,
        current.evicted_bytes + delta.evicted_bytes,
        current.conflicts,
    )
...
def run_once(config: Config, args: argparse.Namespace) -> int:
    ...
    promoted_units = 0
    evict_unprotected = EvictResult()
    evict_prefetch = EvictResult()
    ...
    unit_stats: dict[Path, UnitStats] = {}
    ondeck_entries: list[OnDeckEntry] = []
    ondeck_scores: dict[Path, int] = {}
    desired_ondeck: set[Path] = set()
    current_ondeck_units: set[Path] = set()
    prefetched_ondeck_units: set[Path] = set()
    recent_units: set[Path] = set()
    protected_units: set[Path] = set()

    def refresh_policy_state() -> None:
        nonlocal unit_stats, ondeck_entries, ondeck_scores, desired_ondeck
        nonlocal current_ondeck_units, prefetched_ondeck_units, recent_units, protected_units
        unit_stats = collect_all_unit_stats(config)
        ondeck_entries = try_collect_ondeck_entries(config)
        ondeck_scores = build_ondeck_scores_from_entries(ondeck_entries, config)
        desired_ondeck = select_desired_units(unit_stats, ondeck_scores, config.ondeck_budget_bytes)
        current_ondeck_units, prefetched_ondeck_units = classify_ondeck_units(ondeck_entries, desired_ondeck)
        recent_units = cached_recent_units(unit_stats, config)
        protected_units = desired_ondeck | recent_units

    def cached_policy_sets() -> tuple[set[Path], set[Path], set[Path], set[Path]]:
        cache_units = {unit for unit, stats in unit_stats.items() if stats.size_on_cache > 0}
        current_cached = current_ondeck_units & cache_units
        prefetched_cached = (prefetched_ondeck_units & cache_units) - current_cached
        recent_cached = (recent_units & cache_units) - current_cached - prefetched_cached
        unprotected_cached = cache_units - current_cached - prefetched_cached - recent_cached
        return unprotected_cached, prefetched_cached, recent_cached, current_cached
    ...
    if args.demote_non_frequent and do_evict:
        print("manual mode: demoting cached media outside recent and On Deck protection")
        unprotected_cached, _prefetched_cached, _recent_cached, _current_cached = cached_policy_sets()
        evict_unprotected, evicted_units = apply_evictions(
            sort_units_by_age(unprotected_cached, unit_stats, state),
            evict_unprotected,
            None,
        )
    ...
    if do_sync:
        inline_evict_non_frequent = None
        if args.demote_non_frequent and do_evict:
            unprotected_cached, _prefetched_cached, _recent_cached, _current_cached = cached_policy_sets()
            inline_evict_non_frequent = unprotected_cached
        sync_result, inline_evicted = sync_archive(config, inline_evict_non_frequent)
        conflict_count += sync_result.conflicts
        conflict_count += inline_evicted.conflicts
        evict_unprotected = merge_evict_results(evict_unprotected, inline_evicted)
        refresh_policy_state()

    if do_promote:
        promote_order = sorted(current_ondeck_units, key=lambda unit: (-ondeck_scores.get(unit, 0), str(unit))) + sorted(
            desired_ondeck - current_ondeck_units,
            key=lambda unit: (-ondeck_scores.get(unit, 0), str(unit)),
        )
        missing_ondeck = [
            unit
            for unit in promote_order
            if unit_stats.get(unit) is not None
            and unit_stats[unit].size_on_tank > 0
            and unit_stats[unit].size_on_cache < 1
        ]
    ...
    print(
        ...
        f"recent_cached_units={len(recent_units)} "
        f"desired_ondeck_units={len(desired_ondeck)} "
        f"promoted_units={promoted_units} "
        f"promoted_bytes={format_bytes(promote_result.moved_bytes)} "
        f"evicted_unprotected_units={evict_unprotected.evicted_units} "
        f"evicted_unprotected_bytes={format_bytes(evict_unprotected.evicted_bytes)} "
        f"evicted_prefetch_units={evict_prefetch.evicted_units} "
        f"evicted_prefetch_bytes={format_bytes(evict_prefetch.evicted_bytes)} "
        f"evicted_recent_units={evict_recent.evicted_units} "
        f"evicted_recent_bytes={format_bytes(evict_recent.evicted_bytes)} "
