...
#   migration snapshots taken via run_replication() during HA migrate also flush
#   dirty TXG data first.  Without this patch the replicated path is not covered
#   by the Storage.pm fix because LXC/Migrate.pm skips storage_migrate entirely
#   for volumes already handled by run_replication().
# ---------------------------------------------------------------------------
