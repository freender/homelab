...
PXE_TEMPLATES = ROOT / "pve-pxe" / "templates"
...
def read_template(name: str) -> str:
    return (PXE_TEMPLATES / name).read_text(encoding="utf-8")


def test_httpboot_entrypoint_template_sets_pxe_server_from_mgmt_ip() -> None:
    autoexec = read_template("httpboot-autoexec.ipxe")

    # The entry point bakes in the per-host server IP once, then every
    # downstream chain inherits it via iPXE's persistent ${pxe-server}.
    assert "set pxe-server {{ MGMT_IP }}" in autoexec
    assert "chain http://${pxe-server}/boot.ipxe" in autoexec
    assert "${next-server}" not in autoexec
    assert "http:///" not in autoexec


def test_boot_menu_template_sets_pxe_server_and_chains_through_it() -> None:
    boot = read_template("boot.ipxe")

    assert "set pxe-server {{ MGMT_IP }}" in boot
    assert "chain http://${pxe-server}/pdm-auto-warning.ipxe" in boot
    for name in ("pve-tui", "pve-gui", "pve-debug", "pve-serial"):
        assert f"chain http://${{pxe-server}}/{name}.ipxe" in boot
...
def test_ipxe_scripts_resolve_server_at_runtime_and_avoid_stale_placeholders() -> None:
    for path in PXE_CONFIGS.glob("*.ipxe"):
        ...
        # Menus must not hardcode the server; they inherit ${pxe-server}.
...
def test_pdm_auto_path_sets_proxmox_auto_installer_flag() -> None:
    ...
    assert "chain http://${pxe-server}/pve-load.ipxe" in pdm_auto


def test_shared_loader_loads_kernel_initrd_and_iso_together() -> None:
    loader = read_config("pve-load.ipxe")

    assert "kernel http://${pxe-server}/vmlinuz initrd=initrd.img ${pve-kargs}" in loader
    assert "initrd --name initrd.img http://${pxe-server}/initrd.img" in loader
    assert "initrd http://${pxe-server}/proxmox-ve_PLACEHOLDER.iso" in loader
...
def test_interactive_installer_entries_chain_through_shared_loader() -> None:
    ...
    for name in ("pve-tui.ipxe", "pve-gui.ipxe", "pve-debug.ipxe", "pve-serial.ipxe"):
        ...
        assert "chain http://${pxe-server}/pve-load.ipxe" in script
...
def test_pxe_autoupdate_rewrites_only_iso_filename_and_validates_payload() -> None:
    updater = read_config("pxe-autoupdate")

    # The server is resolved at runtime via ${pxe-server}, so the only
