#!/bin/bash
#set -x
# https://github.com/SpaceinvaderOne/Unraid_ZFS_Dataset_Snapshot_and_Replications Commit 0181fed
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
...
source_dataset_auto_select_exclude_prefix=""	# Prefix to exclude certain datasets from auto-selection. Leave empty to disable exclusion
...
child_destination_dataset="appdata" #works only if source_dataset_auto_select = no
...
####################
#
# Check if mover or age_mover is currently running
is_mover_running() {
    if [ -f /var/run/mover.pid ] && ps h $(cat /var/run/mover.pid) 2>/dev/null | grep -q mover; then
...
#
# Manage Turbowrite mode intelligently
# Args: $1 = "enable" or "disable"
manage_turbowrite() {
    local action="$1"
    
    if [ "$action" = "enable" ]; then
        echo "Enabling Turbowrite"
        /usr/local/sbin/mdcmd set md_write_method 1
    elif [ "$action" = "disable" ]; then
        if is_mover_running; then
            echo "Mover is running - skipping Turbowrite disable (mover will restore it)"
        else
            echo "Disabling Turbowrite"
...
########################################
# Enable Turbowrite
manage_turbowrite "enable"
# Execute the main function to start the process
run_for_each_dataset
# Disable Turbowrite
manage_turbowrite "disable"
