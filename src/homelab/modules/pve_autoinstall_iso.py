...
def deploy(
    ...
    if need_render and not offline_mode():
        ...
        to_render = []
        print_sub(
            "All answer files present on saint; "
            "skipping 1Password render (use --force to refresh)"
        )
    else:
        to_render = iso_hosts
    ...
    if to_render and not offline_mode():
        ...
        try:
            for host in to_render:
                ...
                toml = _build_iso_answer_toml(root, registry, host, global_cfg, pw_hash)
    ...
    if dry_run:
        ...
        rendered = ", ".join(to_render) if to_render else "(none - all present)"
        print_sub(f"  answer files: {rendered}")
...
def validate(
    ...
    try:
        op_secrets.secret_file(root, "pve-deploy-webhook")
